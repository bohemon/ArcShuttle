...
This guide installs ArcShuttle without a source checkout. Commands are pinned to v0.2.0 so an
...
The v0.2.0 Release contains both `ArcShuttle` and the `Parxtract` compatibility module. The
...
`Import-Module Parxtract -RequiredVersion 0.2.0` when required.
